...
public class RouteDa{
    ...
    public String getLatestID(){
        ...
        try{
            ...
            ResultSet rs = null;
            String queryOne = "SELECT * FROM ROUTE ORDER BY ROUTEID";
     ...
     public RouteDo getRecord(String orderid) {
        String queryStr =" select d.scheduleid, d.orderid, d.quantity, d.ticketno, r.price\n " +
                                       " from schedule s, orderdetail d,route r\n " +
                                      " where d.scheduleid = s.scheduleid and s.routeid = r.routeid and orderid=? ";
        RouteDo route = null;
        try {
            ppst = conn.prepareStatement(queryStr, ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
            ppst.setString(1, orderid);
            ResultSet rs = ppst.executeQuery();
            
            rs.afterLast();
            while(rs.previous())
            {
                route=new RouteDo(rs.getString("routeid"));
                break;
            }
